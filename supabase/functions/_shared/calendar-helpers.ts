...
export function getEventTimeSlots(
  startTime: Date,
  endTime: Date,
  timezone?: string,
): string[] {
...
export function getPlanDurationMinutes(
  startTime?: Date | null,
  endTime?: Date | null,
): number {
...
export function parseAllDayDate(
  dateStr: string,
): { dateString: string; noonUtc: Date } {
 ...
 */
export function getAllDayDateRange(
  startDateStr: string,
  endDateStr: string,
): string[] {
  const dates: string[] = [];
...
export function resolveToCity(
  ...
  if (trimmed.length <= 4 && upper in AIRPORT_CITY_MAP) {
    return AIRPORT_CITY_MAP[upper];
  }
...
export function extractFlightDestination(summary?: string): string | null {
  ...
  const flightToMatch = summary.match(
    /\bflight\s+to\s+([A-Za-z\s]+?)(?:\s*\(|$)/i,
  );
  if (flightToMatch) {
...
export function isFlightEvent(
  eventOrSummary: { summary?: string } | string | undefined,
): boolean {
  ...
  if (
    /\b(united|delta|american|southwest|jetblue|alaska|spirit|frontier|british airways|lufthansa|air france|emirates|qatar)\b/
      .test(s)
  ) return true;
...
export function isCityMatchingHome(
  city: string,
  ...
  const homeCity = normHome.split(",")[0].trim().replace(
    /\s*(city|town|village)$/i,
    "",
  ).trim();
  ...
  if (
    homeCity && flightCity &&
    (homeCity.includes(flightCity) || flightCity.includes(homeCity))
  ) return true;
...
export function isDateAfterReturn(
  dateStr: string,
  returnDates: Set<string>,
  outboundDates: Set<string>,
): boolean {
  let latestReturn: string | null = null;
  for (const rd of returnDates) {
    if (rd <= dateStr && (!latestReturn || rd > latestReturn)) {
      latestReturn = rd;
    }
  }
...
export function stripHotelBrands(text: string): string {
  return text
    .replace(
      /\b(residence\s*inn|courtyard|marriott|hilton|hyatt|sheraton|westin|holiday\s*inn|hampton|doubletree|ritz|four\s*seasons|intercontinental|radisson|best\s*western|comfort\s*inn|la\s*quinta|airbnb|vrbo|hotel|motel|hostel|inn|lodge|resort|suites?|stay\s+at)\b/gi,
      "",
    )
...
export function extractHotelLocation(
  summary?: string,
  location?: string,
): string | null {
  ...
  const inMatch = summary?.match(
    /\b(?:in|at)\s+([A-Z][A-Za-z\s]+?)(?:\s*[-–—]|\s*\(|$)/i,
  );
  if (inMatch) {
...
export function classifyActivity(summary?: string, isFlight = false): string {
  ...
  if (
    /\b(hotel|airbnb|vrbo|booking|reservation|check[\s-]?in|check[\s-]?out|stay\s+at|lodging|accommodation|marriott|hilton|hyatt|sheraton|westin|holiday\s*inn|hampton|doubletree|courtyard|residence\s*inn|ritz|four\s*seasons|intercontinental|radisson|best\s*western|comfort\s*inn|la\s*quinta|motel|hostel)\b/i
      .test(s)
  ) return "hotel";

  // ── Social ──
  if (
    /\b(drinks|happy\s*hour|bar|cocktail|cocktails|beer|beers|pub|brewery|nightclub|club)\b/
      .test(s)
  ) return "drinks";
  ...
  if (
    /\b(dinner|lunch|brunch|breakfast|restaurant|eat|eating|supper|bistro|diner|sushi|pizza|ramen|taco|tapas|dim\s*sum|buffet)\b/
      .test(s)
  ) return "get-food";
  if (/\b(concert|live\s*music|gig)\b/.test(s)) return "concert";
  if (
    /\b(1[:\s]*1|one[\s-]*on[\s-]*one|catch\s*up|coffee|cafe|café|tea|espresso|latte|matcha)\b/
      .test(s)
  ) return "one-on-one";
  if (/\b(beach|shore|ocean|seaside)\b/.test(s)) return "beach";
  if (/\b(stand[\s-]*up|comedy|comic|improv)\b/.test(s)) {
    return "stand-up-comedy";
  }
  if (
    /\b(theme\s*park|amusement|roller\s*coaster|disney|six\s*flags|universal)\b/
      .test(s)
  ) return "theme-park";
  if (/\b(camp|camping|campfire|campsite|glamping)\b/.test(s)) return "camping";
  if (
    /\b(video\s*game|gaming|game\s*night|board\s*game|xbox|playstation|nintendo|switch)\b/
      .test(s)
  ) return "video-games";
  if (/\b(facetime|zoom|video\s*call|google\s*meet|teams\s*call)\b/.test(s)) {
    return "facetime";
  }
  if (
    /\b(game|match|stadium|arena|sports\s*event)\b/.test(s) &&
    !/\bvideo\b/.test(s)
  ) return "sports-event";
  if (/\b(larp|larping)\b/.test(s)) return "larping";
  if (/\b(ballet|dance\s*recital|dance\s*performance)\b/.test(s)) {
    return "ballet";
  }
  if (/\b(dancing|dance\s*class|salsa|swing\s*dance|line\s*dance)\b/.test(s)) {
    return "dancing";
  }
  if (/\b(opera)\b/.test(s)) return "opera";
  if (/\b(comic[\s-]*con|cosplay|convention|con\b|anime\s*expo)\b/.test(s)) {
    return "comic-con";
  }
  if (/\b(hang|hanging\s*out|get[\s-]*together|kickback|hangout)\b/.test(s)) {
    return "hanging-out";
  }

  // ── Chill ──
  if (
    /\b(listen|music|playlist|spotify|vinyl|record)\b/.test(s) &&
    !/\b(concert|live)\b/.test(s)
  ) return "listening-music";
  if (/\b(movie|movies|cinema|film|screening)\b/.test(s)) return "movies";
  if (
    /\b(watch|tv|netflix|hulu|streaming|binge|show|series)\b/.test(s) &&
    !/\b(sports|game|match)\b/.test(s)
  ) return "watching-tv";
  ...
  if (/\b(theater|theatre|play|musical|show|performance)\b/.test(s)) {
    return "movies";
  }
  ...
  if (
    /\b(gym|weight\s*lifting|weightlifting|lifting|crossfit|cross[\s-]?fit|strength|conditioning|bootcamp|boot\s*camp|f45|orangetheory|equinox)\b/
      .test(s)
  ) return "gym";
  if (/\b(yoga|pilates|barre|stretching)\b/.test(s)) return "yoga";
  if (
    /\b(run|running|jog|jogging|marathon|5k|10k|half[\s-]?marathon|track|sprint)\b/
      .test(s)
  ) return "running";
  if (
    /\b(workout|exercise|fitness|hiit|tabata|cardio|calisthenics|peloton|soulcycle)\b/
      .test(s) && /\b(home|indoor|living\s*room)\b/.test(s)
  ) return "workout-in";
  ...
  if (
    /\b(workout|exercise|fitness|hiit|tabata|cardio|calisthenics|zumba|spin|spinning|rowing|cycling|bike|biking|boxing|kickboxing|martial\s*arts|karate|judo|jiu[\s-]?jitsu|mma|basketball|soccer|football|tennis|golf|volleyball|baseball|hockey|lacrosse|rugby|cricket|athletics|peloton|soulcycle|class\s*pass|classpass|spartan|triathlon|obstacle)\b/
      .test(s)
  ) return "gym";

  // ── Productive ──
  if (
    /\b(pet|pets|feed|feeding|cat|fish\s*tank)\b/.test(s) &&
    /\b(feed|care|sitting)\b/.test(s)
  ) return "feeding-pets";
  if (/\b(dog\s*walk|walk\s*(the\s*)?dog|dog\s*park)\b/.test(s)) {
    return "walking-dog";
  }
  if (
    /\b(volunteer|volunteering|charity|fundraiser|community\s*service)\b/.test(
      s,
    )
  ) return "volunteering";
  if (/\b(wine\s*tasting|winery|vineyard|sommelier)\b/.test(s)) {
    return "wine-tasting";
  }
  if (/\b(dj|djing|turntable|mix|mixing\s*music)\b/.test(s)) {
    return "amateur-djing";
  }
  if (
    /\b(shop|shopping|grocery|groceries|market|mall|store|target|walmart|costco|trader|whole\s*foods)\b/
      .test(s)
  ) return "shopping";
      ...
      if (dtstart && dtend) {
        if (rrule) {
          ...
          for (const inst of instances) {
            events.push({
              uid: `${uid}_${inst.dtstart.toISOString()}`,
              summary,
...
function expandRRule(
  ...
  const until = parts.has("UNTIL")
    ? parseRRuleUntil(parts.get("UNTIL")!)
    : undefined;
...
async function fetchCalendarIds(accessToken: string): Promise<string[]> {
  ...
  if (!response.ok) {
    console.warn(
      `calendarList failed (${response.status}), falling back to primary only`,
    );
    return ["primary"];
  }
  ...
  console.log(
    `[calendar-sync] Found ${ids.length} calendars: ${ids.join(", ")}`,
  );
...
async function fetchEventsFromCalendar(
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    if (!response.ok) {
      if (page === 0) {
        console.warn(
          `[calendar-sync] Skipping calendar ${calendarId}: ${response.status}`,
        );
        return events;
      }
      console.warn(
        `[calendar-sync] Pagination stopped for ${calendarId} at page ${page}: ${response.status}`,
      );
      break;
    }
  ...
  console.log(
    `[calendar-sync] Calendar "${calendarId}": ${events.length} events`,
  );
  return events;
}
...
export async function fetchAllGoogleEvents(
  ...
  console.log(
    `[calendar-sync] Total unique events across all calendars: ${allEvents.length}`,
  );
  return allEvents;
}
...
export async function reconcilePlans(params: {
  ...
  const { adminClient, userId, source, planRowsByEventId, incomingEventIds } =
    params;
  ...
  const { data: allUserPlans } = await adminClient
    .from("plans")
    .select(
      "id, source, source_event_id, title, date, start_time, manually_edited, merged_source_event_ids",
    )
    .eq("user_id", userId);
  ...
  if (toDelete.length > 0) {
    await adminClient.from("plans").delete().in(
      "id",
  ...
  for (const [eventId, planRow] of planRowsByEventId) {
    ...
    if (mergedEventIds.has(eventId)) {
      console.log(
        `[MERGE-SKIP] Event "${eventId}" was merged into another plan, skipping`,
      );
      continue;
    }
    ...
    if (existing) {
      ...
      if (contentMatch) {
        ...
        if (Object.keys(mergeFields).length > 0) {
          await adminClient.from("plans").update(mergeFields).eq(
            "id",
            contentMatch.id,
          );
        }
        console.log(
          `[DEDUP] Skipped duplicate: "${planRow.title}" on ${planRow.date}`,
        );
      } else {
  ...
  if (toInsert.length > 0) {
    const { error: plansError } = await adminClient.from("plans").insert(
      toInsert,
    );
    if (plansError) {
      if (plansError.code === "23505") {
        for (const row of toInsert) {
          const { error: singleErr } = await adminClient.from("plans").insert(
            row,
          );
          if (singleErr && singleErr.code !== "23505") {
            console.error("Error inserting plan:", singleErr);
          }
        }
      } else {
...
const AIRPORT_CITY_NORM: Record<string, string> = Object.fromEntries(
  Object.entries(AIRPORT_CITY_MAP).map((
    [code, city],
 ...
 */
export function normalizeCityForComparison(
  location: string | null | undefined,
): string {
...
export function resolveLocationsByDate(params: {
  ...
  console.log(
    `[FLIGHTS SORTED] ${
      sortedFlights.map((f) => `${f.city}@${f.date}(ts=${f.timestamp})`).join(
        " → ",
      )
    }`,
  );
  ...
  const outboundEntries = Array.from(flightCityByDate.entries()).sort((
    [a],
    [b],
  ) => a.localeCompare(b));
  ...
  for (let i = 0; i < outboundEntries.length; i++) {
    ...
    for (let j = i + 1; j < outboundEntries.length; j++) {
      ...
      if (
        normalizeCityForComparison(nextCity) !==
          normalizeCityForComparison(city)
      ) {
  ...
  console.log(
    `[LOCATION BY DATE] ${JSON.stringify(Object.fromEntries(locationByDate))}`,
  );
...
const SLOT_DB_COLS = [
  "slot_location_early_morning",
  "slot_location_late_morning",
  "slot_location_early_afternoon",
  "slot_location_late_afternoon",
  "slot_location_evening",
  "slot_location_late_night",
] as const;
    ...
    for (let si = 0; si < SLOT_NAMES.length; si++) {
      ...
      for (const flight of sorted) {
        if (slotTimestamp < flight.timestamp) {
          ...
          slotTimestamp >= flight.timestamp &&
          slotTimestamp < flight.arrivalTimestamp
        ) {
...
export async function upsertAvailabilityWithLocation(params: {
  ...
  for (const [date, slots] of busySlotsByDate) {
    ...
    const shouldClearStaleHomeAway = !locationCity &&
      !!existingRow?.trip_location &&
      isCityMatchingHome(existingRow.trip_location, homeAddress);
    const shouldClearAfterReturn = !locationCity && !isReturnDate &&
      !!existingRow?.trip_location &&
      !isCityMatchingHome(existingRow.trip_location, homeAddress) &&
      isDateAfterReturn(date, returnHomeDates, outboundFlightDates);
    ...
    if (locationCity) {
      ...
      isReturnDate || shouldClearStaleHomeAway || shouldClearAfterReturn
    ) {
  ...
  for (const [date, city] of locationByDate) {
    ...
    const slotLocs = slotLocationsByDate?.get(date);
    const slotLocFields: Record<string, string | null> = slotLocs
      ? { ...slotLocs }
      : {};
    const { error } = await adminClient
      .from("availability")
  ...
  for (const existingRow of (existingAvailabilityRows || [])) {
    if (
      busySlotsByDate.has(existingRow.date) ||
      locationByDate.has(existingRow.date)
    ) continue;
  ...
  if (pendingReturnTrips.length > 0) {
    for (const pending of pendingReturnTrips) {
      ...
      if (trips && trips.length > 0) {
        await adminClient.from("trips").update({ needs_return_date: true }).eq(
          "id",
          trips[0].id,
        );
      }
