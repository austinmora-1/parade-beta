...
BEGIN
  ...
  -- shrink or delete any trip that included this date
  IF NEW.location_status = 'home' OR NEW.trip_location IS NULL THEN
    -- Find trips that span this date for this user
    -- For each such trip, recompute the valid consecutive away-day run
    -- and either shrink or delete the trip
    DELETE FROM public.trips t
    WHERE t.user_id = NEW.user_id
      AND t.start_date <= NEW.date
      AND t.end_date >= NEW.date
      -- Only delete if the trip no longer has 2+ consecutive away days
      AND NOT EXISTS (
        SELECT 1
