-- Allow plan invites to act as named placeholders for friends not yet on Parade
ALTER TABLE public.plan_invites
  ADD COLUMN IF NOT EXISTS placeholder_name text;

-- Index to quickly find unclaimed placeholder invites for a plan
CREATE INDEX IF NOT EXISTS idx_plan_invites_plan_pending_placeholder
  ON public.plan_invites (plan_id)
  WHERE status = 'pending' AND placeholder_name IS NOT NULL;

-- Update accept_plan_invite to attribute the placeholder name when the friend claims the invite.
-- When the accepter signs in and claims a placeholder invite, we save their friendship using
-- the placeholder_name (so the inviter sees the same name they typed when planning),
-- then the friend becomes a real plan_participant.
