...
-- Add per-slot location columns to availability table
ALTER TABLE public.availability
  ADD COLUMN IF NOT EXISTS slot_location_early_morning text,
  ADD COLUMN IF NOT EXISTS slot_location_late_morning text,
  ADD COLUMN IF NOT EXISTS slot_location_early_afternoon text,
  ADD COLUMN IF NOT EXISTS slot_location_late_afternoon text,
  ADD COLUMN IF NOT EXISTS slot_location_evening text,
  ADD COLUMN IF NOT EXISTS slot_location_late_night text;

-- Update get_dashboard_data to include slot location columns
...
BEGIN
  ...
  own_plans AS (
    ...
    FROM public.plans p WHERE p.user_id = p_user_id ORDER BY p.date ASC LIMIT 200
  ),
  ...
  participated_plans AS (
    ...
    WHERE p.user_id <> p_user_id ORDER BY p.date ASC LIMIT 200
  ),
  all_plan_ids AS (
  ...
  outgoing_friend_profiles AS (
    SELECT pr.user_id, pr.avatar_url FROM public.profiles pr
  ...
  incoming_friend_profiles AS (
    SELECT pr.user_id, pr.display_name, pr.avatar_url FROM public.profiles pr
...
-- Drop and recreate get_availability_by_share_code with new return type
DROP FUNCTION IF EXISTS public.get_availability_by_share_code(text, date, date);

CREATE FUNCTION public.get_availability_by_share_code(p_share_code text, p_start_date date, p_end_date date)
