  ...
  return (
    <>
      <div className="space-y-1.5">
        ...
        <div className="flex gap-2 overflow-x-auto -mx-1 px-1 pb-1 snap-x snap-mandatory scrollbar-none">
          {sorted.map((w) => (
            <WindowChip
...
function WindowChip({ window: w, onClick }: { window: OpenWindow; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="snap-start shrink-0 min-w-[150px] max-w-[180px] rounded-xl border border-border bg-card px-3 py-2 text-left shadow-soft hover:border-primary/40 hover:shadow-md transition-all"
    >
      <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
        {w.dayLabel}
      </p>
      <p className="font-display text-sm font-semibold leading-tight mt-0.5 truncate">
      ...
      <div className="mt-1.5 flex items-center gap-1.5 min-h-[18px]">
        {friendCount > 0 ? (
          <>
            <div className="flex -space-x-1.5">
              {w.overlappingFriends.slice(0, 3).map((f) => (
                <Avatar key={f.userId} className="h-4 w-4 ring-1 ring-card">
          ...
          <span className="text-[10px] text-muted-foreground">{w.hours}hr free</span>
        )}
      </div>
    </button>
  );
