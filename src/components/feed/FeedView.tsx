...
import { ACTIVITY_CONFIG, TIME_SLOT_LABELS, FeedVisibility } from '@/types/planner';
import { getPlanDisplayTitle } from '@/lib/planTitle';
import { cn } from '@/lib/utils';
import { MapPin, Clock, Users, CalendarCheck, Camera, Globe, Lock, ChevronDown } from 'lucide-react';
...
export function FeedView() {
  ...
  // Build plan-only feed
  const feedItems = useMemo(() => {
    const items: { data: any; timestamp: Date }[] = [];
    const now = new Date();
    ...
    plans.forEach((plan) => {
      ...
      if (planDate < todayStart && plan.participants && plan.participants.length > 0) {
        items.push({ data: plan, timestamp: planDate });
      }
    });
    ...
    friendPublicPlans.forEach((plan) => {
      ...
      items.push({ data: plan, timestamp: new Date(plan.date) });
    });

    items.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
    return items;
  }, [plans, friendPublicPlans]);

  // Fetch plan photos
  const [planPhotos, setPlanPhotos] = useState<Record<string, string[]>>({});
  const feedPlanIds = useMemo(() => feedItems.map(i => i.data.id), [feedItems]);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Make a plan with friends to get started!
        </p>
      </div>
    );
  }
      ...
      <AnimatePresence initial={false}>
        {visibleItems.map((item, idx) => (
          ...
          >
            <PlanFeedCard
              plan={item.data}
              onClick={() => navigate(`/plan/${item.data.id}`)}
              photos={planPhotos[item.data.id] || []}
      ...
      {hasPhotos && (
        <div className={cn(
          ...
          {photos.slice(0, 4).map((url, i) => (
            <img key={i} src={url} alt="Plan photo" className="w-full h-full object-cover min-h-0" />
          ))}
          {photos.length > 4 && (
            <div className="absolute bottom-2 right-2 rounded-full bg-background/80 backdrop-blur px-2 py-0.5 text-xs font-medium text-foreground flex items-center gap-1">
              <Camera className="h-3 w-3" />+{photos.length - 4}
            </div>
          )}
        </div>
      )}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {isSameDay(plan.date, new Date()) && (
                  <span className="rounded-full bg-primary/15 px-1.5 py-0.5 text-[10px] font-semibold text-primary uppercase tracking-wider">Today</span>
                )}
                {planIsPast && (
                  <span className="rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">Past</span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-1">
              <span className="flex items-center gap-1 text-xs text-muted-foreground">
                ...
                {plan.startTime ? formatTimeRange(plan.startTime, plan.endTime) : timeSlotConfig.time}
              </span>
              {plan.participants.filter((p: any) => p.role !== 'subscriber').length > 0 && (
                <div className="flex items-center -space-x-1.5" onClick={e => e.stopPropagation()}>
                  {plan.participants.filter((p: any) => p.role !== 'subscriber').slice(0, 4).map((p: any, i: number) => (
                    <div key={p.id || i} className="h-5 w-5 rounded-full ring-1 ring-background overflow-hidden shrink-0">
                      <img src={p.avatar || getElephantAvatar(p.name)} alt={p.name} className="h-full w-full object-cover" />
                    </div>
                  ))}
                  {plan.participants.filter((p: any) => p.role !== 'subscriber').length > 4 && (
                    <span className="text-[10px] text-muted-foreground ml-1.5">+{plan.participants.filter((p: any) => p.role !== 'subscriber').length - 4}</span>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="mt-2.5 pt-2 border-t border-border/50 flex items-center justify-between" onClick={e => e.stopPropagation()}>
            <Popover open={visPopoverOpen} onOpenChange={setVisPopoverOpen}>
              ...
              <PopoverContent className="w-44 p-1" side="top" align="start" sideOffset={4}>
                <button onClick={() => handleVisibilityChange('private')} className={cn("flex items-center gap-2 w-full rounded-md px-2.5 py-1.5 text-xs hover:bg-muted transition-colors", currentVisibility === 'private' && "bg-muted font-medium")}>
                  <Lock className="h-3.5 w-3.5" /> Private
                </button>
                <button onClick={() => handleVisibilityChange('friends')} className={cn("flex items-center gap-2 w-full rounded-md px-2.5 py-1.5 text-xs hover:bg-muted transition-colors", currentVisibility === 'friends' && "bg-muted font-medium")}>
                  <Globe className="h-3.5 w-3.5" /> All Friends
                </button>
                {pods.map(pod => (
                  <button key={pod.id} onClick={() => handleVisibilityChange(`pod:${pod.id}`)} className={cn("flex items-center gap-2 w-full rounded-md px-2.5 py-1.5 text-xs hover:bg-muted transition-colors", currentVisibility === `pod:${pod.id}` && "bg-muted font-medium")}>
